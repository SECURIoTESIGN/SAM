...
class Session extends Component{
  state = {
    ...
    session: {
      ...
      q_c_index: 0,   // Current array index of children.
  ...
  */
  ...
  get_tree_depth_nodes = (node, depth) => {
    if (node['children']){
  ...
  handle_answer_selection = (event, answer, index=null) => {
    ...
    // Check if the current answer has sub-questions to be asked (children).
    if (answer['children'].length != 0){
      ...
      // Get the sub question
      let _sub_question = answer['children'][0];
      _sub_question['asked'] = true;

      this.setState({session: {...this.state.session, 
      p_answers: _p_answers,
      question: _sub_question
      }}, () => {
        console.log(this.state.session.p_answers);
      })
      return;
    }
    ...
    if (answer['children'].length == 0){
      ...
      console.log("Current p_answers_index="+ this.state.session.p_answers_index)
      console.log("entrou");
      ...
      for (let i=0; i < this.state.session.p_answers.length; i++){
        ...
        console.log("Going back to answer '" + _previous_answer['name'] + " with sub question");
        // Let's go backwards into the tree, backtracing the path until the first answered that leads us to the current answer, always checking if there any more sub-questions
        // Lets get a sub quesiton to yet questioned
        let flag_childless = false;
        ...
        console.log("Found sub-question to be asked: " + _sub_question);
      ...
      this.setState({session: {...this.state.session, 
        question: _sub_question,
        p_answers_index: this.state.session.p_answers_index+1,
        }}, () => {
          console.log(this.state.session.p_answers);
      })
    }
