...
class Session extends Component{
  state = {
    ...
    session: {
      ...
      q_index: 0,     // Current array index of a root question.
      q_c_index: 0,
  ...
  */
  ...
  handle_answer_selection = (event, answer) => {
    const DEBUG = true;
    if (DEBUG && answer) console_log("handle_answer_selection", "answer selected :" + answer['name']);
    if (DEBUG && !answer) console_log("handle_answer_selection", "answer inputted :" + JSON.stringify(this.state.session.tmp));
    
    let flag_found = false;
    // Does this answer triggers any sub-question ?
    if (thi.state.session.questions.length != 0){
      for(let i=0; i < this.state.session.questions.length; i++){
        let _questions  = this.state.session.questions; 
        let _question   = _questions[i];
        if (_question['trigger'] == answer['id'] && !_question['p']){
          _question['p'] = true;
          console.log("_question['trigger']=" + _question['trigger'] + ", answer['id']=" + answer['id']);
          this.setState({session: {...this.state.session, question: _question, questions: _questions}})
          flag_found = true;
          break;
        }
      }
    ...
    /* Recursively find the parent of targetNode in the tree AKA the answer that triggered the question */
  search_tree = (node, targetNode) => {
      const DEBUG = false;
      
      if (node['type'] == 'answer'){
        if (node['id'] == null && targetNode['id'] == null){
          if (node['client_id'] == targetNode['client_id']){
            if (DEBUG) console.log("[SAM] search_tree() => Found node =" + JSON.stringify(node));
            return(node);
          }
        }else{
      ...
      if (node['children'].length > 0) // paranoic mode
        for(let i=0; i < node['children'].length; i++){
          tmp = this.search_tree(node['children'][i], targetNode, node)
  ...
  // Recus.  get all questions of the tree
  get_all_questions_of_tree = (tree, node, questions) => {
    if (node['type'] == "question") questions.push(node);
    if (node['children']){
      for(let i=0; i < node['children'].length; i++){
        let child = node['children'][i]
        this.get_all_questions_of_tree(tree, child, questions);
      }
    }
  }
  ...
  iterate_tree_root_questions = () => {
    ...
    for(let i=0; i < tree.length; i++){
      let node = tree[i];
      this.get_all_questions_of_tree(tree, node, questions);
    }
    
    console.log(questions)
    let _question = null;
    let q_index   = this.state.session.q_index;
    // Iterate the questions until we found one not yet answered, must of those questions that are inside [questions] are sub-questions triggered by answer.
    do{
      _question = questions[q_index]
      if (!_question['p']) break;
      q_index = q_index + 1;
    }while(true)

    this.setState({session: {...this.state.session, question: _question, questions: questions, q_index: q_index + 1 }}, () => {
