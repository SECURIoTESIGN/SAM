...
import {QuestionIcon} from '../helpers/IconMakerHelper';
import {useStyles} from './ManageQuestionsStyles';
...
//
export const resource_name_singular = "question";
export const resource_name_plural   = "questions";
//
  ...
  render(){
    ...
    return(
      <React.Fragment>
        ...
        <table className={classes.delete}>
          ...
          <tbody><tr>
            <td >
              ...
              <u>Be aware</u>, by removing a question, further existing links will also be removed. In particular, if there are sessions linked to the current question, those will be <u>permanently deleted.</u>
              </Typography>
            </td>
          </tr></tbody>
          <tbody><tr>
        ...
        <PopupComponent popupIcon={<QuestionIcon color="disabled"/>} title={this.state.selected_id ? "Edit Question" : "Add Question"}  open={this.state.open_add_edit} onClose={() => this.setState({open_add_edit: false})} TransitionComponent={Transition}>
          <QuestionComponent resource={this.state.selected_id} onClose={() => window.location.reload(false)} />
        </PopupComponent>
        ...
        <div className={classes.root}>
          ...
          <table border="0">
          <tbody><tr>
            <td><QuestionIcon color="disabled"/></td>
          ...
          <SelectionComponent edit={true} delete={true} 
            ...
            type={"questions"}/><br/>
          <Button id="b_add_module" name="b_add_module" startIcon={<QuestionIcon/>} onClick={() => this.setState({open_add_edit: true, selected_id: null})}  color="primary" variant="contained">Add New {resource_name_singular}</Button>
        </div>
      </React.Fragment>
    );
  }
...
export default withStyles(useStyles)(ManageQuestions)